...
class Qwen2MoePolicy(InferenceV2Policy):
    ...
    def instantiate_model(
        self, engine_config: RaggedInferenceEngineConfig, mp_group: Any
    ) -> Qwen2MoeInferenceModel:
        return Qwen2MoeInferenceModel(
            config=self._model_config,
            engine_config=engine_config,
            base_mp_group=mp_group,
        )
    ...
    def build_container_map(self) -> ContainerMap:
        ...
        transformer_containers = [
            Qwen2MoeTransformerContainer(self.model)
            for _ in range(self.model.num_layers)
        ]

        map.set_transformer_params(["model.layers"], transformer_containers)

        map.set_non_transfoWrmer_params(Qwen2MoeNonTransformerContainer(self.model))
