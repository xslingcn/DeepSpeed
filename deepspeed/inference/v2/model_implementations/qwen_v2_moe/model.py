...
class Qwen2MoeInferenceModel(DSMoETransformerModelBase):
    ...
    def make_moe_layer(self) -> None:
        ...
        sharded_dim = sharded_intermediate_dim(
            self.intermediate_dim // self.n_top_k, self.tp_size, self.tp_rank
        )
    ...
    def make_shared_expert_mlp_1_layer(self) -> None:
        ...
        self.shared_expert_mlp_1 = heuristics.instantiate_linear(
    ...
    def make_shared_expert_mlp_2_layer(self) -> None:
        ...
        self.shared_expert_mlp_2 = heuristics.instantiate_linear(
            linear_config, self._engine_config
        )
    ...
    def make_shared_expert_gate_layer(self) -> None:
        ...
        """
        shard_size = sharded_intermediate_dim(
            self.model_dim, self.tp_size, self.tp_rank
        )
        ...
        self.shared_expert_gate = heuristics.instantiate_linear(
            linear_config, self._engine_config
        )
    ...
    def __init__(
        self,
        config: DSModelImplementationConfig,
    ...
    def _forward_embed(self, ragged_batch: RaggedBatchWrapper) -> torch.Tensor:
        ...
        if embed.shape[-1] != self.model_dim  :
            raise ValueError(
                f"Embedding output shape {embed.shape} does not match model_dim {self.model_dim}"
            )
    ...
    def _forward_transformer(
        ...
        residual, hidden_states = self.norm(
            residual, hidden_states, cur_params.mlp_norm_gamma, beta=None
        )

        shared_expert_output = self.shared_expert_mlp_1(
            hidden_states, cur_params.shared_moe_mlp_1, b=None
        )
        shared_expert_output = self.shared_expert_mlp_2(
            shared_expert_output, cur_params.shared_moe_mlp_2, b=None
        )
        shared_expert_gate_output = self.shared_expert_gate(
            hidden_states, cur_params.shared_moe_gate, b=None
        )[..., :1]
        ...
        if layer_idx != self.num_layers - 1:
            ...
            residual, hidden_states = self.norm(
                residual, hidden_states, next_params.attn_norm_gamma, beta=None
            )
        else:
    ...
    def _forward_unembed(
        self, hidden_states: torch.Tensor, ragged_batch_info: RaggedBatchWrapper
    ) -> torch.Tensor:
        """
        ...
        if self.tp_size > 1:
            comm_buffer = empty_from(
                self._comm_logits, (self.tp_size, logits.shape[0], logits.shape[1])
            )
            full_logits = empty_from(
                self._return_logits, (logits.shape[0], self.vocab_size)
            )

            dist.all_gather_into_tensor(comm_buffer, logits, group=self._base_mp_group)

            full_logits.copy_(
                comm_buffer.permute(1, 0, 2).reshape(logits.shape[0], self.vocab_size)
            )
    ...
    def forward(self, wrapped_batch: RaggedBatchWrapper) -> torch.Tensor:
        ...
        residual, hidden_states = self.norm(
            residual, None, self._transformer[0].attn_norm_gamma, beta=None
        )

        for layer_idx in range(self.num_layers):
            residual, hidden_states = self._forward_transformer(
